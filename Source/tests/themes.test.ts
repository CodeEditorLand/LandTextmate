...
class ThemeInfo {
	...
	public create(resolver: Resolver): ThemeData {
		...
		if (this._includeFilename) {
			let includeTheme: IRawTheme = ThemeInfo._loadThemeFile(
				this._includeFilename
			);
			(<any>theme).settings = includeTheme.settings.concat(
				theme.settings
			);
		}
...
(function () {
	...
	let _grammars: IGrammarRegistration[] = JSON.parse(
		fs
			.readFileSync(path.join(THEMES_TEST_PATH, "grammars.json"))
			.toString("utf8")
	);
	...
	let _languages: ILanguageRegistration[] = JSON.parse(
		fs
			.readFileSync(path.join(THEMES_TEST_PATH, "languages.json"))
			.toString("utf8")
	);
...
test("Theme matching gives higher priority to deeper matches", () => {
	...
	assert.deepStrictEqual(
		theme.getColorMap()[actual!.foregroundId],
		"#300000"
	);
});
...
suite("Theme matching can match", () => {
	...
	test("simpleMatch24", () =>
		assert.deepStrictEqual(match("storage.object"), {
			fontStyle: "not set",
		}));
	test("simpleMatch25", () =>
