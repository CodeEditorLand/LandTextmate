...
import {
	StateStackImpl as StackElementImpl,
...
if (process.argv.length < 4) {
	console.log(
		"usage: node index.js <mainGrammarPath> [<additionalGrammarPath1> ...] <filePath>"
	);
	process.exit(0);
}
...
Promise.all(grammarPromises).then((_grammars) => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		let stackElement: ExtendedStackElement | null = <ExtendedStackElement>(
			r.ruleStack
		);
		let cnt = 0;
		while (stackElement) {
