...
function assertTokenizationSuite(testLocation: string): void {
	...
	let tests: IRawTest[] = JSON.parse(
		fs.readFileSync(testLocation).toString()
	);
	...
	async function performTest(
		...
		let grammar: IGrammar | null =
			await registry.loadGrammar(grammarScopeName);
		if (!grammar) {
			throw new Error("I HAVE NO GRAMMAR FOR TEST");
		}
		let prevState: StateStack | null = null;
		for (let i = 0; i < test.lines.length; i++) {
			prevState = assertLineTokenization(
				grammar,
				test.lines[i],
				prevState
			);
