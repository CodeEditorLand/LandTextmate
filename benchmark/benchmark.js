...
const onigurumaRegistry = new Registry({
	loadGrammar,
	onigLib: onigLibs.getOniguruma(),
});
...
async function tokenizeFile(filePath, scope, message) {
	...
	console.log(message);
	console.log(
		"TOKENIZING " + content.length + " lines using grammar " + scope
	);
...
function loadGrammar(scopeName) {
	...
	if (scopeName === "source.js") {
		...
		grammarPath = path.resolve(
			__dirname,
			"..",
			"test-cases/themes/syntaxes/JSON.json"
		);
	} else {
		return null;
	}
	return Promise.resolve(
		main.parseRawGrammar(
...
async function test() {
	await tokenizeFile(
		path.join(__dirname, "large.js.txt"),
		"source.js",
		"jQuery v2.0.3"
	);
	await tokenizeFile(
		path.join(__dirname, "bootstrap.css.txt"),
		"source.css",
		"Bootstrap CSS v3.1.1"
	),
		await tokenizeFile(
			path.join(__dirname, "vscode.d.ts.txt"),
			"source.ts",
			"vscode.d.ts"
		);
	await tokenizeFile(
		path.join(__dirname, "JavaScript.tmLanguage.json.txt"),
		"source.ts",
		"JSON"
	);
	await tokenizeFile(
		path.join(__dirname, "bootstrap.min.css.txt"),
		"source.css",
		"Bootstrap CSS v3.1.1 minified"
	);
	await tokenizeFile(
		path.join(__dirname, "large.min.js.txt"),
		"source.js",
		"jQuery v2.0.3 minified"
	);
	await tokenizeFile(
		path.join(__dirname, "main.08642f99.css.txt"),
		"source.css",
		"Bootstrap with multi-byte minified"
	);
	await tokenizeFile(
		path.join(__dirname, "minified.js.txt"),
		"source.js",
		"Simple minified file"
	);
}
test();
